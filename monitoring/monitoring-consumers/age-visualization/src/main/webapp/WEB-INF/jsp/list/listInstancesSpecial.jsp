...
<div id="middle">
    <fieldset>
        ...
        <a href="${pageContext.request.contextPath}/age/list/${computationType}/last/${gathererId}">dispaly last
            data</a>

        <c:set var="count" value="${fn:length(cdColl)}" scope="page"/>
        <form:form
                action="/age/showChecked?computationType=${computationType}&computationInstance=*&gathererId=${gathererId}"
                method="POST" commandName="checkedStatistics">
            <c:forEach items="${cdColl}" var="cd">
                <form:checkbox path="checkboxNumber" value="${count}"/>
                ${cd.toString()}
                <a href="${pageContext.request.contextPath}/age/list/${computationType}/${cd.toString()}/${gathererId}">display
                    data</a>
                <a href="${pageContext.request.contextPath}/age/show/${computationType}/${cd.toString()}/${gathererId}">show
                    single</a>
