package org.hage.platform.simulationconfig.distribution.division;

import com.google.common.collect.ImmutableMap;
import com.google.common.primitives.UnsignedInteger;
import org.fest.assertions.Assertions;
import org.hage.platform.component.container.definition.IComponentDefinition;
import org.hage.platform.simulationconfig.CellPopulationDescription;
import org.hage.platform.simulationconfig.ComputationConfiguration;
import org.hage.platform.simulationconfig.HabitatGeography;
import org.hage.platform.simulationconfig.PopulationDistributionMap;
import org.hage.platform.habitat.AgentDefinition;
import org.hage.platform.component.simulation.structure.InternalPosition;
...
public class SimpleConfigurationAllocatorTest {

    private ConfigurationDivisor tested;
    ...
    public void setUp() throws Exception {
        tested = new ConfigurationDivisor();
    }
    ...
    public void shouldNotSplitWithProportionsWithOneCountable() throws Exception {
        ...
        ConfigurationDivision configDivision = tested.divideUsingProportions(originalConfiguration, proportions);

        // then

        assertThat(configDivision.getFor(countable).orElse(DEFAULT_CONFIG)).isEqualTo(originalConfiguration);
    ...
    public void shouldDivideConfigurationIntoComplementaryParts() throws Exception {
        ...
        ConfigurationDivision configDivision = tested.divideUsingProportions(configuration, forCountable(firstCountable, secondCountable));

        PopulationDistributionMap firstMap = configDivision.getFor(firstCountable).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap();
        PopulationDistributionMap secondMap = configDivision.getFor(secondCountable).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap();
    ...
    public void shouldDivideBigConfigurationIntoComplementaryParts() throws Exception {
        ...
        ConfigurationDivision configurationDivision = tested.divideUsingProportions(configuration, forCountable(countable));

        PopulationDistributionMap actualMergedMap = countable.stream()
            .map(configurationDivision::getFor)
    ...
    public void shouldSplitConfigurationSavingTotalOrderingOfProportions() throws Exception {
        ...
        for (Countable c1 : countable) {
            PopulationDistributionMap c1DistrMap = configurationDivision.getFor(c1).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap();
            final Long c1DistrMapNumberOfAgents = c1DistrMap.getNumberOfAgents();

            for (Countable c2 : countable) {
                PopulationDistributionMap c2DistrMap = configurationDivision.getFor(c2).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap();
    ...
    public void shouldContainConfigurationForAllCountableFromProportions() throws Exception {
        ...
        final List<Countable> countable = range(1, 1000)
            .mapToObj(SimpleConfigurationAllocatorTest::countableFor)
            .collect(toList());
        ...
        ConfigurationDivision division = tested.divideUsingProportions(configuration, forCountable(countable));

        // then

        countable.forEach(
            c -> assertThat(division.getFor(c).isPresent()).isTrue()
        );
    ...
    public void shouldNotSplitGlobalComponents() throws Exception {
        ...
        ConfigurationDivision configDivision = tested.divideUsingProportions(configuration, proportions);

        // then

        assertThat(configDivision.getFor(firstCountable).orElse(DEFAULT_CONFIG).getGlobalComponents()).containsOnly(firstGlobalComp, secondGlobalComp, thirdGlobalComp);
        assertThat(configDivision.getFor(secondCountable).orElse(DEFAULT_CONFIG).getGlobalComponents()).containsOnly(firstGlobalComp, secondGlobalComp, thirdGlobalComp);
        assertThat(configDivision.getFor(thirdCountable).orElse(DEFAULT_CONFIG).getGlobalComponents()).containsOnly(firstGlobalComp, secondGlobalComp, thirdGlobalComp);
    ...
    public void shouldSaveAtomicityOfInternalPositionPopulationDescriptionDuringSplitting() throws Exception {
        ...
        ConfigurationDivision configDivision = tested.divideUsingProportions(configuration, forCountable(countable));
        ...
        for (InternalPosition internalPosition : internalPositions) {
            ...
            for (Countable c : countable) {
                PopulationDistributionMap mapForCountable = configDivision.getFor(c).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap();
    ...
    public void shouldDivideConfigurationSavingStructureDefinition() throws Exception {
        ...
        assertThat(configDivision.getFor(firstCountable).orElse(DEFAULT_CONFIG).getHabitatGeography().getStructureDefinition()).isEqualTo(structureDefinition);
        assertThat(configDivision.getFor(secondCountable).orElse(DEFAULT_CONFIG).getHabitatGeography().getStructureDefinition()).isEqualTo(structureDefinition);
    ...
    public void shouldReturnEmptyConfigurationForCountableNotInProportions() throws Exception {
        ...
        ConfigurationDivision configDivision = tested.divideUsingProportions(DEFAULT_CONFIG, proportions);

        // then

        assertThat(configDivision.getFor(mock(Countable.class)).isPresent()).isFalse();
    ...
    public void shouldDivideConfigurationIntoTwoPartsOfEqualNumberOfAgents() throws Exception {
        ...
        assertThat(configDivision.getFor(firstCountable).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap().getNumberOfAgents()).isEqualTo(10L);
        assertThat(configDivision.getFor(secondCountable).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap().getNumberOfAgents()).isEqualTo(10L);
    ...
    public void shouldDivideConfigurationIntoThreePartsWithDifferentProportionsOfAgentsNumber() throws Exception {
        ...
        ConfigurationDivision configDivision = tested.divideUsingProportions(configuration, forCountable(c5, c8, c7));

        // then

        assertThat(configDivision.getFor(c5).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap().getNumberOfAgents()).isEqualTo(5L);
        assertThat(configDivision.getFor(c8).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap().getNumberOfAgents()).isEqualTo(8L);
        assertThat(configDivision.getFor(c7).orElse(DEFAULT_CONFIG).getHabitatGeography().getPopulationDistributionMap().getNumberOfAgents()).isEqualTo(7L);
