...
import org.hage.platform.node.execution.phase.StepPhaseExecutor;
...
class StepPhaseExecutionLoggingAspect {
    ...
    @Before(value = "org.hage.platform.node.execution.aspect.Pointcuts.stepPhaseExecution() && target(executor) && args(phase, step)", argNames = "executor,phase,step")
    ...
    @AfterReturning(value = "org.hage.platform.node.execution.aspect.Pointcuts.stepPhaseExecution() && target(executor) && args(phase, step)", argNames = "executor,phase,step")
