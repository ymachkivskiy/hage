...
import org.hage.platform.node.execution.monitor.ExecutionDurationObserver;
...
class ExecutionPhaseDurationMeasureAspect {
    ...
    @Before("org.hage.platform.node.execution.aspect.Pointcuts.stepPhaseExecution()")
    private void startStopWatch() {
        stopwatch.reset();
        stopwatch.start();
    }

    @AfterThrowing(value = "org.hage.platform.node.execution.aspect.Pointcuts.stepPhaseExecution() && args(phase,..)", throwing = "exception")
    ...
    @After("org.hage.platform.node.execution.aspect.Pointcuts.stepPhaseExecution() && args(phase,..)")
