...
public class ConfigurationTranslator {
    ...
    private LoadBalanceConfig mapBalanceConfig(org.hage.platform.simconf.load.xml.LoadBalanceConfig xmlLoadBalanceConfig) {

        if (xmlLoadBalanceConfig.getDisabled() != null) {
            return new LoadBalanceConfig(BalanceMode.DISABLED, 0);
        }

        org.hage.platform.simconf.load.xml.LoadBalanceConfig.AmountType amountType = xmlLoadBalanceConfig.getAmountType();

        if (amountType.getMode() == org.hage.platform.simconf.load.xml.BalanceMode.AFTER_STEP_COUNT) {
